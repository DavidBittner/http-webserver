...
impl From<std::io::Error> for NegotiationError {
    fn from(err: std::io::Error) -> Self { NegotiationError::IoError(err) }
}
...
impl<'a, 'b> ContentNegotiator<'a, 'b> {
    ...
    pub fn best_choice(&self) -> Result<Vec<PathBuf>, NegotiationError> {
        ...
        let types: RankedEntryList<Mime> =
            RankedEntryList::new_list(self.headers.get(ACCEPT).unwrap_or(""))
                .unwrap();
        ...
        let paths = types.filter(paths, |path, entry| {
            ...
            if entry.type_() == desc.type_() || entry.type_() == "*" {
                entry.subtype() == "*" || entry.subtype() == desc.subtype()
            } else {
                false
            }
        });
        ...
        let paths =
            langs.filter(paths, |path, entry| *entry == map_file(path).lang);
        ...
        let mut paths = encodings.filter(paths, |path, entry| {
            *entry == map_file(path).enc.unwrap_or("".into())
        });
