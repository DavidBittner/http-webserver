...
impl Into<ResponseData> for Vec<u8> {
    fn into(self) -> ResponseData { ResponseData::Buffer(self) }
}
...
impl Response {
    fn error(code: StatusCode, desc: &str, mut headers: HeaderList) -> Self {
        ...
        match data {
            Ok(string) => {
                ...
                headers.content("text/html".into(), None, data.len());
    ...
    pub fn range_not_satisfiable() -> Self {
        Response::error(
            ...
            "The given range was outside of the bounds of the requested \
             entity.",
            HeaderList::response_headers(),
        )
    }
    ...
    pub fn timed_out() -> Self {
        ...
        headers.connection(connection::CLOSE.into());
    ...
    pub fn not_modified(loc: &Path) -> Self {
        ...
        let new_path = loc.strip_prefix(&CONFIG.root).unwrap_or(loc);

        let temp = if loc.starts_with(&CONFIG.root) {
            loc.into()
        } else {
            CONFIG.root.join(loc.strip_prefix("/").unwrap_or(loc))
        };
    ...
    fn file_response(path: &Path) -> Self {
        ...
        match std::fs::File::open(path) {
            Ok(file) => {
                ...
                match fs::metadata(path) {
                    Ok(meta) => {
                        ...
                        match time {
                            ...
                            Err(err) => {
                                error!(
                                    "error occured while retrieving modified \
                                     time: '{}'",
                                    err
                                );
                ...
                let desc = map_file(path);
                headers.content_language(&desc.lang);
    ...
    pub fn path_response(path: &Path, req: &Request) -> Self {
        for redir in REDIRECTS.iter() {
            let temp = path.strip_prefix(&CONFIG.root).unwrap();
        ...
        //trailing slashes.
        if path.is_dir() {
            let ends_with = path.as_os_str().to_string_lossy().ends_with("/");
            ...
            if ends_with {
                for file in CONFIG.indexes.iter() {
                    ...
                    if temp.exists() {
                        ...
                        match canon {
                            Ok(path) => {
                                return Response::path_response(&path, req);
                            }
                            Err(err) => {
                ...
                return Response::redirect(&path, StatusCode::MovedPermanently);
            }
        } else if req.headers.has(RANGE) {
            use std::io::ErrorKind::*;

            let range_str = req.headers.get(RANGE).unwrap();

            let ranges: Result<RangeList, _> = range_str.parse();
            match ranges {
                Ok(ranges) => match Self::partial_content(path, ranges) {
                    Ok(resp) => resp,
                    Err(err) => {
                        error!(
            ...
            if path.exists() {
                ...
                let list =
                    ContentNegotiator::new(path, &req.headers).best_choice();

                match list {
                    Ok(mut list) => Self::file_response(&list.pop().unwrap()),
                    Err(err) => match err {
                        NotAcceptable => Self::not_acceptable(),
                        MultipleResponses(list) => {
                            let mut headers = HeaderList::response_headers();
                            let alt = format_alternates(list);
                            headers.alternates(alt);

                            Self::multiple_choices(headers)
                        }
                        _ => Self::not_found(),
                    },
                }
    ...
    fn redirect(path: &Path, code: StatusCode) -> Self {
        let mut headers = HeaderList::response_headers();
        let new_path = path.strip_prefix(&CONFIG.root).unwrap_or(path);

        let temp = if path.starts_with(&CONFIG.root) {
            path.into()
        } else {
            CONFIG.root.join(path.strip_prefix("/").unwrap_or(path))
        };
        ...
        T: std::io::Write + Sized,
    {
        ...
        match self.data {
            Some(dat) => {
                use ResponseData::*;
                match dat {
                    Buffer(buff) => Self::write_w_timeout(writer, &buff)?,
                    Stream(mut stream) => {
                        use std::io::ErrorKind;
...
fn map_lang(path: &Path, desc: &mut FileDescriptor) {
    if let Some(ext) = path.extension() {
        ...
        let lang = match ext.as_str() {
            "en" | "es" | "de" | "ja" | "ko" | "ru" => Some(ext.into()),
            _ => {
